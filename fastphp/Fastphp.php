...
class Fastphp
{
    ...
    public function route()
    {
        ...
        $url = $_SERVER['REQUEST_URI'];//从全局变量$_SERVER中获得uri http://www.baidu.com/qqq/ppp?a=1&b=2 域名后面的东西
        $position = strpos($url, '?');
        $url = $position === false ? $url : substr($url, 0, $position); //  /qqq/ppp
        $url = trim($url,'/');
        if ($url) {
            $paramArray = explode('/', $url);
            $paramArray = array_filter($paramArray);
            $controllerName = ucfirst($paramArray[0]);

            array_shift($paramArray);
            $actionName = $paramArray ? $paramArray[0] : $actionName;

            array_shift($paramArray);
            $param = $paramArray ? $paramArray : $param;
